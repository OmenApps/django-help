...
class DjangoHelpConfig(AppConfig):
    ...
    def ready(self):
        ...
        @register()
        def check_extra_languages_in_settings(app_configs, **kwargs):
            ...
            for lang_code in app_settings.EXTRA_LANGUAGES:
                if lang_code not in available_languages:
                    errors.append(
                        Warning(
                            f"Language '{lang_code}' specified in EXTRA_LANGUAGES is not available in "
                            "settings.LANGUAGES.",
        ...
        @register()
        def check_languages_in_extra_languages(app_configs, **kwargs):
        ...
        @register()
        def check_authorize_user_to_view_article(app_configs, **kwargs):
        ...
        @register()
        def check_intended_entity_type(app_configs, **kwargs):
        ...
        @register()
        def check_base_manager(app_configs, **kwargs):
        ...
        @register()
        def check_base_queryset(app_configs, **kwargs):
        ...
        @register()
        def check_base_model(app_configs, **kwargs):
        ...
        @register()
        def check_base_foreign_key(app_configs, **kwargs):
        ...
        @register()
        def check_extra_languages(app_configs, **kwargs):
        ...
        @register()
        def check_extra_languages_values(app_configs, **kwargs):
        ...
        @register()
        def check_extra_languages_values_keys(app_configs, **kwargs):
        ...
        @register()
        def check_extra_languages_values_blank(app_configs, **kwargs):
            ...
            for lang_code, lang_settings in extra_languages.items():
                ...
                if not isinstance(blank, bool):
                    errors.append(
                        Warning(
                            f"Value for 'blank' key in EXTRA_LANGUAGES value for language '{lang_code}' is not a "
                            "boolean.",
        ...
        @register()
        def check_intended_entity_type_values(app_configs, **kwargs):
        ...
        @register()
        def check_intended_entity_type_values_length(app_configs, **kwargs):
            ...
            for entity_type, entity_settings in intended_entity_type.items():
                if len(entity_settings) != 2:
                    errors.append(
                        Warning(
                            f"Value for entity type '{entity_type}' in INTENDED_ENTITY_TYPE does not have a length "
                            "of 2.",
        ...
        @register()
        def check_intended_entity_type_values_types(app_configs, **kwargs):
        ...
        @register()
        def check_intended_entity_type_values_unique(app_configs, **kwargs):
        ...
        @register()
        def check_intended_entity_type_values_valid(app_configs, **kwargs):
            """Check that values in INTENDED_ENTITY_TYPE tuples are valid."""
            errors = []
            intended_entity_type = app_settings.INTENDED_ENTITY_TYPE
            valid_values = ("person", "organization")

            for entity_type, entity_settings in intended_entity_type.items():
                if entity_settings[0] not in valid_values:
                    errors.append(
                        Warning(
                            f"Invalid value '{entity_settings[0]}' for entity type '{entity_type}' in "
